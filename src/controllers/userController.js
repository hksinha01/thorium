...
const UserDeleted = async function (req, res) {
  ...
  try{
    ...
    let userId = req.params.userId 
   // let deletedData = await userModel.updateMany({_id: userId},{$set :{isDeleted :true}})
    let deletedData = await userModel.findOneAndUpdate({_id: userId},{$set :{isDeleted :true}})
    //let deletedData = await userModel.updateOne({_id: userId},{$set :{isDeleted :true}})
    res.status(200).send('Data is deleted....Check Database')
 }
 catch(error)
 {
   xyz.status(500).send({msg : "Error",error : error.message})
