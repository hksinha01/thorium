...
router.post("/players/:playerName/bookings/:bookingId", function(res,req)
{    let count = true
    ...
    if(count === true)
    {
        for(let i = 0;i < players.length; i++)
    {     
        if(players[i].bookingNumber === bookingId)
        {        count = false
                break;
        }
    }
    ...
    {   let  obj = {}
     ...
     {   
         ...
         console.log("booking was already processed:")
     }
     
    }
    else{
          res.send("something irrelivant about the user ,user can't be found")
    }
})
